...
SITE_URL="${SITE_URL:-${NEXT_PUBLIC_SITE_URL:-https://ducaibao.ink}}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "Usage: npm run cf:init -- [--with-kv] [--site-url=https://ducaibao.ink] [--db-name=qiaomu-blog-db] [--r2-name=qiaomu-blog-images] [--kv-name=qiaomu-blog-cache]" >&2
      exit 1
      ;;
  esac
done

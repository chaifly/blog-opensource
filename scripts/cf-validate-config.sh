...
if [[ ! -f "${CONFIG_PATH}" ]]; then
  echo "鉂?Wrangler config not found: ${CONFIG_PATH}" >&2
  exit 1
fi
...
require_binding() {
  ...
  if ! rg -q "^[[:space:]]*${section_pattern}[[:space:]]*$" "${CONFIG_PATH}"; then
    echo "鉂?Missing ${description} section in ${CONFIG_PATH}" >&2
    exit 1
  fi

  if ! rg -q "^[[:space:]]*${binding_pattern}[[:space:]]*$" "${CONFIG_PATH}"; then
    echo "鉂?Missing ${description} binding in ${CONFIG_PATH}" >&2
    exit 1
  fi
}
...
if ! rg -q 'NEXT_PUBLIC_SITE_URL[[:space:]]*=[[:space:]]*"https?://[^"]+"' "${CONFIG_PATH}"; then
  echo "鉂?Missing NEXT_PUBLIC_SITE_URL in ${CONFIG_PATH}" >&2
  exit 1
fi

if rg -q 'NEXT_PUBLIC_SITE_URL[[:space:]]*=[[:space:]]*"https?://(localhost|127\.0\.0\.1|0\.0\.0\.0|example\.com)(:[0-9]+)?/?\"' "${CONFIG_PATH}"; then
  echo "鉂?NEXT_PUBLIC_SITE_URL points to a local or placeholder host in ${CONFIG_PATH}" >&2
  exit 1
fi
